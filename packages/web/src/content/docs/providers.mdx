...
The `/connect` command now walks you through the whole setup — you no longer need to hand-edit `opencode.json` for a basic OpenAI-compatible provider.
...
2. Enter a unique ID for the provider, then your API key.
   ...
   The API key is stored in your credential file, never written into `opencode.json`.

3. Enter the base URL, a display name, and the model id(s) you want to use.
   ...
   │
   ◇  Provider name
   │  My AI Provider Display Name
   │
   ◇  Model id(s), comma-separated
   │  my-model-name, another-model
   └
   ```

   OpenCode writes the provider configuration to your global config for you.

4. Run the `/models` command and your custom provider and models will appear in the selection list.
...
You can edit this block by hand afterwards to tune advanced options (see the example below).

:::tip
The whole flow is also available from the CLI, including non-interactively:

```bash
opencode auth login -p myprovider --name "My AI Provider Display Name" \
  --base-url https://api.myprovider.com/v1 --model my-model-name,another-model
```
